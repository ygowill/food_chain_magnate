...
func on_map_mode_changed(mode: String, payload: Dictionary) -> void:
	...
	match m:
		"marketing":
			...
			var hint := "点击地图选择位置｜右侧确认"
			if mt == "airplane":
				hint = "点击地图边缘选择位置｜角落需选择横/竖飞｜右侧确认"
		...
		"restaurant_placement":
			...
			_map_mode_bar.call("show_mode", title2, "点击地图选择位置｜R 旋转｜右侧确认")
		"house_placement":
			var action_id2 := str(payload.get("action_id", ""))
			var title3 := "🏠 放置房屋" if action_id2 != "add_garden" else "🌳 添加花园"
			if action_id2 == "add_garden":
				_map_mode_bar.call("show_mode", title3, "点击地图选择房屋｜右侧选择方向并确认")
			else:
				_map_mode_bar.call("show_mode", title3, "点击地图选择位置｜R 旋转｜右侧确认")
		"distance_tool":
			_map_mode_bar.call("show_mode", "📏 距离工具", "支持道路↔道路，或房屋+餐厅｜点起点再点终点｜测完后点击任意目标重开｜D 关闭")
		_:
			_map_mode_bar.call("show_mode", "模式：%s" % m, "")
