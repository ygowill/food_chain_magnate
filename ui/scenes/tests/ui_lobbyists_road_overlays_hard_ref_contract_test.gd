class_name UiLobbyistsRoadOverlaysHardRefContractTest
extends RefCounted

const _ROOT_DIR := "res://ui"
const _EXCLUDE_DIR := "res://ui/scenes/tests/"
const _PATTERN := "res://modules/lobbyists/road_overlays.gd"

static func run() -> Result:
	var files: Array[String] = []
	var list_r := _list_gd_files_recursive(_ROOT_DIR, files)
	if not list_r.ok:
		return list_r
	...
	for path in files:
		...
		var idx := text.find(_PATTERN)
		if idx >= 0:
			var line_no := _find_line_number(text, idx)
			return Result.failure("UI 不应硬引用 lobbyists overlays 脚本: %s:%d" % [path, line_no])
