...
static func _test_newly_trained_employee_filtered_without_milestone(seed_val: int) -> Result:
	...
	var reserve_counts := controller._build_employee_type_counts(Array(state.players[0].get("reserve_employees", [])))
	if not reserve_counts.has("campaign_manager"):
		engine.dispose()
		return Result.failure("首轮培训后，reserve_counts 中应包含 campaign_manager 以复现 UI 过滤场景")

	var filtered := controller._filter_sources_with_valid_targets(state, 0, reserve_counts)
	if filtered.has("campaign_manager"):
		engine.dispose()
...
static func _test_chain_train_source_kept_with_milestone(seed_val: int) -> Result:
	...
	var reserve_counts := controller._build_employee_type_counts(Array(state.players[0].get("reserve_employees", [])))
	var filtered := controller._filter_sources_with_valid_targets(state, 0, reserve_counts)
	if not filtered.has("campaign_manager"):
		engine.dispose()
