class_name UiModulesBaseDirContractTest
extends RefCounted

const _ROOT_DIR := "res://ui"
const _EXCLUDE_DIR := "res://ui/scenes/tests/"
const _FORBIDDEN := "res://modules"
...
static func run() -> Result:
	...
	for path in files:
		...
		var idx := text.find(_FORBIDDEN)
		if idx >= 0:
			var line_no := _find_line_number(text, idx)
			return Result.failure("核心 UI 不应写死 modules base_dir 字符串: %s:%d (%s)" % [path, line_no, _FORBIDDEN])
