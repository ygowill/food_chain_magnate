class_name UiBasePiecesLogoHardRefContractTest
extends RefCounted

const _ROOT_DIR := "res://ui"
const _EXCLUDE_DIR := "res://ui/scenes/tests/"
const _PATTERN := "res://modules/base_pieces/assets/map/logos/"
...
static func run() -> Result:
	...
	for path in files:
		...
		if idx >= 0:
			...
			return Result.failure("UI 不应硬编码 base_pieces logo 贴图路径: %s:%d" % [path, line_no])
