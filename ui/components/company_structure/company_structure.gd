...
const CompanyStructureBuilderClass = preload("res://ui/components/company_structure/company_structure_builder.gd")
...
var _builder = CompanyStructureBuilderClass.new()
...
func _get_display_structure() -> Array:
	...
	if structure_val is Array:
		...
		return _builder.build_strict_structure_from_employees_with_preferred_direct(
			_ceo_slots,
			employees,
			preferred_direct,
			preferred_reports_by_slot,
			Callable(self, "_get_employee_def")
		)

	# 未写入 structure：用当前在岗员工生成“严格金字塔结构”作为展示预览
	return _builder.build_strict_structure_from_employees(_ceo_slots, employees, Callable(self, "_get_employee_def"))
