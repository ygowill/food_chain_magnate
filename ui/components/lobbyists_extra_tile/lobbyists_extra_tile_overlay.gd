...
@onready var tile_option: OptionButton = $HintMargin/HintPanel/VBox/ControlsRow/TileOption
...
func _ready() -> void:
	...
	if tile_option != null and not tile_option.item_selected.is_connected(_on_tile_selected):
		tile_option.item_selected.connect(_on_tile_selected)
...
func _rebuild_tile_options() -> void:
	if tile_option == null:
		return
	tile_option.clear()
	for id in _available_tiles:
		tile_option.add_item(str(id))
	if not _available_tiles.is_empty():
		var idx := _available_tiles.find(_selected_tile_id)
		tile_option.select(maxi(0, idx))

func _on_tile_selected(index: int) -> void:
	if index < 0 or index >= _available_tiles.size():
		return
	_selected_tile_id = _available_tiles[index]
	_emit_highlight_request()
	_update_ui()
	ui_state_changed.emit()
