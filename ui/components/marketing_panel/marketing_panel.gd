...
var _type_buttons: Dictionary = {}  # type_id -> MarketingTypeButton
...
func _rebuild_type_buttons() -> void:
	...
	for type_def in MARKETING_TYPES:
		...
		var btn := MarketingTypeButton.new()
		btn.type_id = type_id
...
func _on_type_selected(type_id: String) -> void:
	...
	for tid in _type_buttons.keys():
		var btn: MarketingTypeButton = _type_buttons[tid]
		if is_instance_valid(btn):
...
# === 内部类：营销类型按钮 ===
class MarketingTypeButton extends PanelContainer:
