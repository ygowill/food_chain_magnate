...
- JSON: `res://testdata/saves/manual_cases/milestones/first_burger_marketed.json`
- 玩家数: 2
- Seed: 12345
...
- 验证发起营销动作 `InitiateMarketing(product=burger)` 会触发里程碑 first_burger_marketed。

## 复核步骤

1. 载入后应处于 Working/Marketing，且玩家 0 在岗包含 marketing_trainee。
2. 行动面板选择「发起营销」，执行一次营销：`board_number=14 product=burger duration=1 position=[6,1] rotation=0`。

## 预期结果

- 玩家 0 获得里程碑 first_burger_marketed（player.milestones）。
- `state.map.marketing_placements['14'].product == 'burger'`。
