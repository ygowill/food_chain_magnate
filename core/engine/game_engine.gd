...
func execute_command(command: Command, is_replay: bool = false) -> Result:
	...
	return Result.success(state).with_warnings(execute_result.warnings)
...
func rewind_to_command(target_index: int) -> Result:
	...
	var replay_result := ReplayClass.rewind_to_command(command_history, checkpoints, action_registry, target_index)
...
func full_replay() -> Result:
	...
	var replay_result := ReplayClass.full_replay(command_history, checkpoints, action_registry)
	if not replay_result.ok:
		return replay_result
