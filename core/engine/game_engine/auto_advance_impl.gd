...
static func try_advance_one(state_in: GameState, phase_manager: PhaseManager, action_registry: ActionRegistry) -> Result:
	...
	if state_in.round_number == 1 and state_in.phase == "Restructuring":
		var blocked_r: Result = _is_phase_blocked_by_pending_actions(state_in, "Restructuring")
		if not blocked_r.ok:
			return blocked_r
	...
	if state_in.round_number == 1 and state_in.phase == "OrderOfBusiness":
		var blocked_r2: Result = _is_phase_blocked_by_pending_actions(state_in, "OrderOfBusiness")
		if not blocked_r2.ok:
			return blocked_r2
		if bool(blocked_r2.value):
			return Result.success(false)

		var fin: Result = _auto_finalize_order_of_business_round1(state_in)
		if not fin.ok:
			return fin
	...
	if state_in.phase == "Restructuring":
		var blocked_r4: Result = _is_phase_blocked_by_pending_actions(state_in, "Restructuring")
	...
	if state_in.phase == "OrderOfBusiness":
		var blocked_r5: Result = _is_phase_blocked_by_pending_actions(state_in, "OrderOfBusiness")
	...
	if _is_auto_skip_settlement_phase(state_in.phase):
		var blocked_r3: Result = _is_phase_blocked_by_pending_actions(state_in, str(state_in.phase))
	...
	if state_in.phase == "Working":
		...
		var mandatory_r: Result = _try_auto_complete_working_mandatory_actions(state_in, action_registry)
