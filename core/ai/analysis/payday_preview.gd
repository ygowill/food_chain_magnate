...
static func preview_after_commands(
	...
	if engine == null:
		return Result.failure("PaydayPreview.preview_after_commands: engine is null")
	...
	for i in range(commands.size()):
		...
		if command == null:
			_restore_source_registries(engine)
			return Result.failure("PaydayPreview.preview_after_commands: command[%d] is null" % i)
		var command_copy := command.duplicate_command()
		var exec_read := _execute_command_for_preview(fork, command_copy)
		if not exec_read.ok:
			_restore_source_registries(engine)
			return Result.failure("PaydayPreview.preview_after_commands: command[%d] failed: %s" % [i, exec_read.error])
	...
	if max_steps <= 0:
		_restore_source_registries(engine)
		return Result.failure("PaydayPreview.preview_after_commands: max_steps must be positive")

	for _step in range(max_steps):
		var state := fork.get_state()
		if state == null:
			_restore_source_registries(engine)
			return Result.failure("PaydayPreview.preview_after_commands: fork state is null")

		var fallback_read := _build_fallback_command(fork)
		if not fallback_read.ok:
			var failure := Result.failure("PaydayPreview.preview_after_commands: cannot reach Payday report: %s" % fallback_read.error).with_value({
				"engine": fork,
				"state": state,
		...
		if not exec_fallback.ok:
			_restore_source_registries(engine)
			return Result.failure("PaydayPreview.preview_after_commands: fallback failed: %s" % exec_fallback.error)
	...
	var max_steps_failure := Result.failure("PaydayPreview.preview_after_commands: max_steps reached before Payday report").with_value({
		"engine": fork,
...
static func _execute_command_for_preview(engine: GameEngine, command: Command) -> Result:
	...
	if str(command.action_id) == ActionIdsClass.ADVANCE_PHASE and str(state.phase) == DefsClass.PHASE_PAYDAY:
		return _execute_payday_advance_without_auto_skip(engine, command)
	return engine.execute_command(command)

static func _execute_payday_advance_without_auto_skip(engine: GameEngine, command: Command) -> Result:
	var state := engine.get_state()
	if state == null:
		return Result.failure("state is null")
	if str(state.phase) != DefsClass.PHASE_PAYDAY:
		return Result.failure("not in Payday")
	if not str(state.sub_phase).is_empty():
		return Result.failure("Payday sub_phase is not empty")
	var target := str(command.params.get("target", "phase"))
	if target != "phase":
		return Result.failure("Payday direct advance only supports target=phase")
	if not _command_validates(engine, command):
		return Result.failure("Payday advance command does not validate")
...
static func _try_extract_report(engine: GameEngine) -> Result:
	if engine == null:
		return Result.failure("PaydayPreview._try_extract_report: engine is null")
	var state := engine.get_state()
	if state == null:
		return Result.failure("PaydayPreview._try_extract_report: state is null")
	if not (state.round_state is Dictionary):
		return Result.failure("PaydayPreview._try_extract_report: round_state is not Dictionary")
	var report_val = Dictionary(state.round_state).get("payday", null)
	if not (report_val is Dictionary):
		return Result.failure("PaydayPreview._try_extract_report: round_state.payday is missing")
	var report: Dictionary = Dictionary(report_val).duplicate(true)
	for key in ["base_due", "discount", "milestone_delta", "due", "paid", "unpaid", "details"]:
		if not report.has(key):
			return Result.failure("PaydayPreview._try_extract_report: payday.%s is missing" % key)
...
static func _build_payload(
	...
	return {
		...
		"base_due": _copy_array(report.get("base_due", [])),
		"discount": _copy_array(report.get("discount", [])),
		"milestone_delta": _copy_array(report.get("milestone_delta", [])),
		"due": _copy_array(report.get("due", [])),
		"paid": _copy_array(report.get("paid", [])),
		"unpaid": _copy_array(report.get("unpaid", [])),
		"details": _copy_array(report.get("details", [])),
...
static func _build_fallback_command(engine: GameEngine) -> Result:
	...
	match str(state.phase):
		...
		DefsClass.PHASE_MARKETING, DefsClass.PHASE_CLEANUP:
			return Result.failure("Payday already passed")
		_:
