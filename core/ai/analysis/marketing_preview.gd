...
static func preview_after_commands(
	...
	if engine == null:
		return Result.failure("MarketingPreview.preview_after_commands: engine is null")
	...
	for i in range(commands.size()):
		...
		if command == null:
			_restore_source_registries(engine)
			return Result.failure("MarketingPreview.preview_after_commands: command[%d] is null" % i)
		var command_copy := command.duplicate_command()
		var exec_read := _execute_command_for_preview(fork, command_copy)
		if not exec_read.ok:
			_restore_source_registries(engine)
			return Result.failure("MarketingPreview.preview_after_commands: command[%d] failed: %s" % [i, exec_read.error])
	...
	if max_steps <= 0:
		_restore_source_registries(engine)
		return Result.failure("MarketingPreview.preview_after_commands: max_steps must be positive")

	for _step in range(max_steps):
		var state := fork.get_state()
		if state == null:
			_restore_source_registries(engine)
			return Result.failure("MarketingPreview.preview_after_commands: fork state is null")

		var fallback_read := _build_fallback_command(fork)
		if not fallback_read.ok:
			var failure := Result.failure("MarketingPreview.preview_after_commands: cannot reach Marketing: %s" % fallback_read.error).with_value({
				"engine": fork,
				"state": state,
		...
		if not exec_fallback.ok:
			_restore_source_registries(engine)
			return Result.failure("MarketingPreview.preview_after_commands: fallback failed: %s" % exec_fallback.error)
	...
	var max_steps_failure := Result.failure("MarketingPreview.preview_after_commands: max_steps reached before Marketing report").with_value({
		"engine": fork,
...
static func _try_extract_report(engine: GameEngine) -> Result:
	if engine == null:
		return Result.failure("MarketingPreview._try_extract_report: engine is null")
	var state := engine.get_state()
	if state == null:
		return Result.failure("MarketingPreview._try_extract_report: state is null")
	if not (state.round_state is Dictionary):
		return Result.failure("MarketingPreview._try_extract_report: round_state is not Dictionary")
	var report_val = Dictionary(state.round_state).get("marketing", null)
	if not (report_val is Dictionary):
		return Result.failure("MarketingPreview._try_extract_report: round_state.marketing is missing")
	var report: Dictionary = Dictionary(report_val).duplicate(true)
	for key in ["rounds", "processed", "expired", "timeline_events"]:
		if not report.has(key):
			return Result.failure("MarketingPreview._try_extract_report: marketing.%s is missing" % key)
...
static func _build_payload(
	...
	return {
		...
		"processed": _copy_array(report.get("processed", [])),
		"expired": _copy_array(report.get("expired", [])),
		"timeline_events": _copy_array(report.get("timeline_events", [])),
...
static func _build_fallback_command(engine: GameEngine) -> Result:
	...
	match str(state.phase):
		...
		DefsClass.PHASE_MARKETING:
			return Result.failure("Marketing reached without report")
		DefsClass.PHASE_CLEANUP:
			return Result.failure("Marketing already passed")
		_:
