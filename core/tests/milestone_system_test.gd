...
static func run(player_count: int = 2, seed_val: int = 12345) -> Result:
	...
	var r14 := _test_guru_can_continue_training_after_two_step(seed_val)
	if not r14.ok:
		return r14

	var r15 := _test_switch_trainer_disallowed_without_milestone(seed_val)
	if not r15.ok:
		return r15

	var r16 := _test_switch_between_two_coaches_disallowed_without_milestone(seed_val)
	if not r16.ok:
		return r16
