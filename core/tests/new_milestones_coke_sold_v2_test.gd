...
static func run(player_count: int = 2, seed_val: int = 889900) -> Result:
	...
	if str(task.get("kind", "")) != "fridge_keep" or int(task.get("player_id", -1)) != 0:
		return Result.failure("pending_phase_actions[Cleanup] 应为 fridge_keep(player=0)，实际: %s" % str(pending))
