...
static func run() -> Result:
	...
	var prev_email_verified := bool(PlatformSession.email_verified)
	var prev_email_verification_pending := bool(PlatformSession.email_verification_pending)
	...
	if str(NetContext.player_profile.get("name", "")) != "MyNick":
		_restore(prev_profile, prev_user_id, prev_session_id, prev_is_guest, prev_display_name, prev_email, prev_email_verified, prev_email_verification_pending, prev_is_admin, prev_created_at)
...
static func _restore(
	...
	prev_email: String,
	prev_email_verified: bool,
	prev_email_verification_pending: bool,
	prev_is_admin: bool,
	...
	if PlatformSession != null:
		...
		PlatformSession.email_verified = prev_email_verified
		PlatformSession.email_verification_pending = prev_email_verification_pending
