...
static func _test_parse_configs() -> Result:
	...
	if int(strategic_options.get("mcts_iterations", 0)) != 10:
	...
	if int(tuning_strategic_options.get("mcts_max_depth", 0)) != 2:
		return Result.failure("tuning matrix should pass through strategic mcts depth option: %s" % str(tuning_parsed.value))
	if int(tuning_strategic_options.get("mcts_root_prior_min_visits_per_child", 0)) != 3:
		return Result.failure("tuning matrix should pass through strategic mcts root prior guard option: %s" % str(tuning_parsed.value))
