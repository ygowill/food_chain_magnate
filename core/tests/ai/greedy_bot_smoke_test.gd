...
static func run(_player_count: int = 2, seed_val: int = 12345) -> Result:
	...
	if not second.ok:
		return second
	var prefix := _action_prefix(longer_smoke.value.get("actions", []), Array(second.value.get("actions", [])).size())
	if str(prefix) != str(second.value.get("actions", [])):
		return Result.failure("GreedyBot should be deterministic for same seed prefix")
	return Result.success({
		"steps": int(second.value.get("steps", 0)),
...
static func _action_prefix(actions_val, count: int) -> Array:
	var actions: Array = actions_val if actions_val is Array else []
	var out := []
	for i in range(mini(maxi(0, count), actions.size())):
