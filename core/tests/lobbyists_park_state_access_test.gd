...
static func _test_validate_specific_fails_fast_on_missing_supply(seed_val: int) -> Result:
	...
	state.map.erase("lobbyists_park_line_supply_remaining")
	var action = ActionClass.new()
	var command := Command.create("place_lobbyists_park", 0)
	command.params = {"piece_id": "lobbyists_park_line"}
	var result := action._validate_specific(state, command)
	if result.ok:
		return Result.failure("缺失公园 supply 时应失败")
	var err := str(result.error)
	if err.find("state.map.lobbyists_park_line_supply_remaining") < 0:
		return Result.failure("错误信息应包含 state.map.lobbyists_park_line_supply_remaining，实际: %s" % err)
	return Result.success()
...
static func _test_apply_changes_fails_fast_without_partial_mutation(seed_val: int) -> Result:
	...
	if result.ok:
		return Result.failure("缺失公园 supply 时 _apply_changes 应失败")
	var err := str(result.error)
	...
	var counts_val = state.round_state.get("lobbyists_place_counts", null)
	if counts_val is Dictionary and not (counts_val as Dictionary).is_empty():
		return Result.failure("失败时不应提前写入 lobbyists_place_counts")
	return Result.success()

static func _find_valid_command(state: GameState) -> Result:
	var action = ActionClass.new()
	var grid: Vector2i = state.map["grid_size"]
	for piece_id in ActionClass.PARK_PIECES:
