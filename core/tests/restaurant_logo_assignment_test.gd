...
static func run(_player_count: int = 3, seed_val: int = 12345) -> Result:
	...
	var s0_read := GameStateClass.create_initial_state_with_rng(_player_count, seed_val, rng0, cfg, [])
	if not s0_read.ok:
	...
	var seen0 := {}
	for pid in range(_player_count):
	...
	var s1_read := GameStateClass.create_initial_state_with_rng(_player_count, seed_val, rng1, cfg, [])
	if not s1_read.ok:
	...
	var choices: Array[int] = [2, -1, 0]
	var rng2 := RandomManager.new(seed_val)
	var s2_read := GameStateClass.create_initial_state_with_rng(3, seed_val, rng2, cfg, choices)
	if not s2_read.ok:
	...
	var s3_read := GameStateClass.create_initial_state_with_rng(3, seed_val, rng3, cfg, choices)
	if not s3_read.ok:
	...
	var dup_choices: Array[int] = [1, 1, -1]
	var rng4 := RandomManager.new(seed_val)
	var s4_read := GameStateClass.create_initial_state_with_rng(3, seed_val, rng4, cfg, dup_choices)
	if not s4_read.ok:
