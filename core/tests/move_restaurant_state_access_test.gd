# move_restaurant 餐厅记录状态访问回归测试
class_name MoveRestaurantStateAccessTest
extends RefCounted
...
static func run(player_count: int = 2, seed_val: int = 12345) -> Result:
	...
	if not r.ok:
		return r
	r = _test_validate_fails_fast_on_invalid_restaurant_cells_without_partial_mutation(player_count, seed_val)
	if not r.ok:
		return r
...
static func _test_validate_fails_fast_on_invalid_restaurant_owner_without_partial_mutation(player_count: int, seed_val: int) -> Result:
	...
	state.map["restaurants"][rest_id]["owner"] = "bad"
	...
	if result.ok:
		return Result.failure("owner 类型错误时 move_restaurant validate 应失败")
	var err := str(result.error)
	if err.find("restaurants[%s].owner" % rest_id) < 0:
		return Result.failure("错误信息应包含 owner 路径，实际: %s" % err)
...
static func _test_validate_fails_fast_on_invalid_restaurant_cells_without_partial_mutation(player_count: int, seed_val: int) -> Result:
	var built := _build_move_restaurant_working_engine(player_count, seed_val)
	if not built.ok:
		return built
	var engine: GameEngine = built.value
	...
	if cmd == null:
		return Result.failure("找不到合法的餐厅移动点")
	state.map["restaurants"][rest_id]["cells"] = "bad"
	var player_before := str(state.players[actor])
	var map_before := str(state.map)
	var round_state_before := str(state.round_state)
	var executor = engine.action_registry.get_executor("move_restaurant")
	var result := executor._validate_specific(state, cmd)
	if result.ok:
		return Result.failure("cells 类型错误时 move_restaurant validate 应失败")
	var err := str(result.error)
...
static func _test_apply_fails_fast_on_invalid_restaurant_cells_without_partial_mutation(player_count: int, seed_val: int) -> Result:
	var built := _build_move_restaurant_working_engine(player_count, seed_val)
	if not built.ok:
		return built
	var engine: GameEngine = built.value
	...
	if result.ok:
		return Result.failure("cells 类型错误时 move_restaurant apply 应失败")
	var err := str(result.error)
	if err.find("restaurants[%s].cells" % rest_id) < 0:
		return Result.failure("错误信息应包含 cells 路径，实际: %s" % err)
...
static func _find_first_valid_place(engine: GameEngine, actor: int) -> Command:
	var state := engine.get_state()
