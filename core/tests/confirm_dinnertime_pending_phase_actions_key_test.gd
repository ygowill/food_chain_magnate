...
static func _case_legacy_global_confirm(player_count: int, seed_val: int) -> Result:
	...
	if not new_state_r.ok:
		return Result.failure("legacy confirm_dinnertime 计算新状态失败: %s" % new_state_r.error)

	return _assert_dinnertime_pending_phase_key_cleared(new_state_r.value, "legacy")
