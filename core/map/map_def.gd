...
const MapDefParserClass = preload("res://core/map/map_def_parser.gd")
...
static func from_dict(data: Dictionary) -> Result:
	var fields_read := MapDefParserClass.parse_fields_from_dict(data)
	if not fields_read.ok:
		return fields_read
	var f: Dictionary = fields_read.value

	var map_def := MapDef.new()
	map_def.id = str(f.get("id", ""))
	map_def.display_name = str(f.get("display_name", ""))
	map_def.grid_size = f.get("grid_size", Vector2i(0, 0))
	map_def.min_players = int(f.get("min_players", 0))
	map_def.max_players = int(f.get("max_players", 0))
	map_def.random_tile_pool = f.get("random_tile_pool", [])
	map_def.random_rotation = bool(f.get("random_rotation", true))
	map_def.random_seed = int(f.get("random_seed", 0))
	map_def.tiles = f.get("tiles", [])
