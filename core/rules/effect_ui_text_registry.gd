...
static func configure_from_ruleset(ruleset) -> Result:
	...
	return _configure_from_entries(effect_items, milestone_items)

static func configure_from_module_ui_metadata(module_ui_metadata) -> Result:
	if not _loaded:
		return Result.failure("EffectUiTextRegistry 未初始化：请先调用 reset()")
	if module_ui_metadata == null:
		return Result.failure("EffectUiTextRegistry.configure_from_module_ui_metadata: module_ui_metadata 为空")
	if not (module_ui_metadata is Object):
		return Result.failure("EffectUiTextRegistry.configure_from_module_ui_metadata: module_ui_metadata 类型错误（期望 Object）")
	if not module_ui_metadata.has_method("get_effect_ui_text_entries"):
		return Result.failure("EffectUiTextRegistry.configure_from_module_ui_metadata: module_ui_metadata 缺少 get_effect_ui_text_entries")
	if not module_ui_metadata.has_method("get_milestone_effect_ui_text_entries"):
		return Result.failure("EffectUiTextRegistry.configure_from_module_ui_metadata: module_ui_metadata 缺少 get_milestone_effect_ui_text_entries")

	var effect_items = module_ui_metadata.get_effect_ui_text_entries()
	if not (effect_items is Array):
		return Result.failure("EffectUiTextRegistry.configure_from_module_ui_metadata: effect_ui_text_entries 缺失或类型错误（期望 Array）")
	var milestone_items = module_ui_metadata.get_milestone_effect_ui_text_entries()
	if not (milestone_items is Array):
		return Result.failure("EffectUiTextRegistry.configure_from_module_ui_metadata: milestone_effect_ui_text_entries 缺失或类型错误（期望 Array）")
...
static func _configure_from_entries(effect_items, milestone_items) -> Result:
	if not (effect_items is Array):
		return Result.failure("EffectUiTextRegistry: effect_ui_texts 缺失或类型错误（期望 Array）")
	if not (milestone_items is Array):
		return Result.failure("EffectUiTextRegistry: milestone_effect_ui_texts 缺失或类型错误（期望 Array）")
