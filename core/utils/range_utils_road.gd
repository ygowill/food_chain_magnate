...
# 目的：拆分 RangeUtils 的职责，避免单文件过大；本文件专注于 road range。
extends RefCounted
...
static func get_min_road_distance_to_any_road_cells(
	...
	for rest_id in restaurant_ids:
		...
		for ep in entrance_points:
			var start_cells_result: Result = get_adjacent_road_cells(state, ep)
...
static func is_within_road_range(
	...
	var target_cells_result: Result = get_adjacent_road_cells(state, target_pos)
	...
	for rest_id in restaurant_ids:
		...
		var start_cells_result: Result = get_adjacent_road_cells(state, entrance_pos)
		if not start_cells_result.ok:
			return start_cells_result
