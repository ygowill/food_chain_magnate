...
static func get_adjacent_road_cells_for_positions(state: GameState, positions: Array) -> Result:
	...
	for i in range(positions.size()):
		...
		var read := get_adjacent_road_cells(state, p)
		if not read.ok:
			return read
...
static func is_within_road_range_to_any_road_cells(
	...
	var min_r := get_min_road_distance_to_any_road_cells(state, actor, restaurant_ids, target_road_cells)
	if not min_r.ok:
		return min_r
...
static func get_min_road_distance_to_any_road_cells(
	...
	for rest_id in restaurant_ids:
		...
		for ep in entrance_points:
			var start_cells_result := get_adjacent_road_cells(state, ep)
