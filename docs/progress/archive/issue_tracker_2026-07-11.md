...
> **已归档（2026-07-11）**：本文是只读原始工作日志，不再代表当前状态，也不得继续追加。当前活跃项见 [`docs/BACKLOG.md`](../../BACKLOG.md)，待人工验收见[验收队列](../acceptance_queue.md)。
...
| 80 | 晚餐售出：现金变化日志需显示收入来源拆分（售价/花园/公园/CFO等） | UI/日志 | 现金变化事件仅记录总额；缺少可追溯的收入拆分来源 | Implemented（待手动验收） |
...
## 80. 晚餐售出：现金变化日志需显示收入来源拆分（售价/花园/公园/CFO等）

**需求**

- 玩家在晚餐时间售出日志中，跟随的现金变化事件需要：
	- 标出本次收入总额；
	- 详细标注总额的来源拆分（至少包含：食物售价、花园加成、公园加成、CFO 加成等；若存在其它来源也应展示）。

**验收**

- 晚餐结算相关的收入日志能看到“总额 + 拆分项列表”，且拆分项的和等于总额。

**实施记录**

- 事件构建：`gameplay/replay/command_runner_event_build/cash_events.gd` 生成 `PLAYER_CASH_CHANGED` 时，在 Dinnertime 上下文为正向现金变化附加 `income_breakdown`（可覆盖进入/离开/阶段内执行结算等场景）。
- 日志展示：`ui/scenes/game/game_event_log_formatter.gd` 已支持按 `income_breakdown.items` 输出“晚餐收入来源：食物售价/花园加成/公园加成/CFO 加成等”。
...
## 81. 动作面板：生产食物不使用下拉框，改为食物图标点击

**需求**

- 玩家动作面板中生产食物时，不使用下拉框选择食物；改为直接展示食物图标供点击选择。

**验收**

- 可点击图标完成选择；选中态明确；不影响生产逻辑与 headless 测试稳定性。

**现状/实施记录**

- 当前实现已为“多选食物”员工提供图标化选择：
	- `ui/components/production_panel/production_panel.gd`：`_build_food_controls()` 使用 `HFlowContainer` + `FoodTypeItem`（图标+名称）而非下拉框。
	- `FoodTypeItem` 选中态通过边框高亮体现，点击即切换选择。
...
## 82. 玩家信息面板：左侧餐厅图标复用顺位餐厅图标；tab 移至上方横排

**需求**

- 玩家信息面板左侧图标复用玩家顺位中的餐厅图标（同背景色）；注意缩放与边界，不得溢出当前范围。
- 玩家信息面板的餐厅图标 tab 放到信息面板上方横向排列。

**验收**

- 图标样式一致、背景色一致、缩放不溢出；tab 位于上方横排且交互正常。

**实施记录**

- 右侧玩家信息面板（`PlayerPanel`）改为：
	- 顶部横排餐厅 logo tab（复用顺位显示的 logo 与 `#f4edd1` 背景色），点击切换 `view_player`；
	- 下方展示当前查看玩家的简要信息，左侧 badge 使用同款餐厅 logo（自动缩放不溢出）。
- 涉及文件：
...
## 83. 日志面板：回放播放器宽度溢出导致横向溢出

**现象**

- 回放播放器宽度大于日志面板宽度，导致出现横向溢出/横向滚动。

**验收**

- 日志面板内无横向溢出；回放播放器在不同窗口宽度下可用且不遮挡。

**实施记录**

- `ReplayBar` 改为横向 `EXPAND_FILL`，并开启 `clip_contents`，避免其最小宽度/子控件布局撑开导致横向溢出：
	- `ui/components/game_log/replay_bar.tscn`
...
## 84. 顶部工具栏：供应堆与里程碑按钮无法打开

**现象**

- 游戏最上方工具栏中的“供应堆”和“里程碑”入口无法打开对应面板。
...
- 玩家参数选择：1-N，不允许“当前玩家”，但允许填玩家 id。
- 把 player 写进命令参数本身（而不是依赖面板中“当前玩家是谁”）。
- 彻底移除“依赖当前玩家”的输入/逻辑（以 UI 形态体现）。
- 房屋选择下拉：显示“房号 + house_id + 坐标”。
- 营销相关下拉：显示 marketing 的 `board_number` 以及对应的类型和尺寸。
- 输入不合法时：允许执行并报错（不必在 UI 端强制拦截）。

**验收**

- 所有与玩家相关的 debug 命令都必须显式指定 player_id；不依赖面板状态；房屋/广告牌/类似对象均使用下拉展示当前对局可用项。

**实施记录**

- 参数弹窗支持“下拉 + 手动输入”的组合控件（不在 UI 端强制拦截无效输入，允许执行并由命令返回错误）：
	- `ui/scenes/debug/components/param_dialog.gd`
- 命令面板的玩家参数改为“玩家顺位 1..N + 允许输入 id:<player_id>”，且不提供“当前玩家”选项：
	- `ui/scenes/debug/tabs/command_tab.gd`
- 房屋下拉文案按“房号 + house_id + 坐标”展示；营销板件下拉按“board_number + 类型 + 尺寸”展示：
	- `ui/scenes/debug/tabs/command_tab.gd`
...
## 86. 说客模组：扩边奖励改为获得里程碑时弹窗二选一（必须当场选择），不作为动作按钮

**需求（已澄清）**

- 触发点：获得里程碑时弹窗（B）。
- 选择方式：必须当场二选一（使用 / 放弃），不允许取消。
- 放置机会与说客动作面板中的动作分离（不要作为动作按钮混在一起）。
...
- 路线只要包含施工标记所在格就应 +1 距离；包含起点格的施工标记也必须计入。

**验收**

- 在起点格放置施工标记时，距离惩罚仍然生效（含自动化测试覆盖）。

**实施记录**

- 距离惩罚计算遍历 `ctx.path` 的所有格（包含起点格 `path[0]`），只要路径包含施工标记所在格就累计 +1：
	- `modules/lobbyists/rules/entry.gd`
- 补充自动化用例覆盖“起点格包含施工标记仍计入惩罚”：
	- `core/tests/lobbyists_v2_test.gd`
...
## 88. 公园/花园：背景色无透明度（花园 #699055；公园 #587a51）
