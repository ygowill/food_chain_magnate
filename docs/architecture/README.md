...
6. `docs/architecture/30-core-engine.md`：`GameEngine`（命令执行、时间线、校验点、存档、回放）
...
14. `docs/architecture/34-core-events.md`：`EventBus`（事件历史与“重建历史”用法）
...
17. `docs/architecture/37-core-rules.md`：规则系统（Settlement/Effects/各类 registry 与模块扩展点）
...
20. `docs/architecture/42-gameplay-replay-timelines.md`：派生时间线（StepTimeline / EventTimeline，用于日志/回放/复盘）
21. `docs/architecture/50-tools-replay.md`：开发工具（`tools/replay_runner.gd`）
