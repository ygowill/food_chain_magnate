...
- `gameplay/replay/step_timeline_build/build_full_impl.gd`（~289 LOC；（已整改 2026-01-27）将阶段切换归属与 auto-advance 分段分别抽离到 `phase_transition.gd`/`auto_advance_drain.gd`，主文件更聚焦于 orchestrator）
  ...
  - 这类逻辑更像 UI/回放子系统的“派生视图构建”，因此已移出 core/engine 并进一步拆分。
...
- `core/engine/game_engine.gd`（~285 LOC；（已整改 2026-01-27）命令索引推导抽离到 `command_index_queries.gd`；回退/回放逻辑抽离到 `rewind_ops.gd`；移除未使用 preload 并删除薄 `_` wrapper）
  - 重逻辑已基本抽离；主文件目前以 orchestrator + 对外 API 为主，后续若继续拆分可按“初始化/存档/调试查询”分层。
...
| `gameplay/replay/step_timeline_build/build_full_impl.gd` | 289 | 6 | 0 | moved:gameplay,uses:EventBus |
| `gameplay/replay/step_timeline_build/auto_advance_drain.gd` | 107 | 4 | 0 | moved:gameplay,helper:step_timeline_auto_advance_drain |
| `gameplay/replay/step_timeline_build/phase_transition.gd` | 150 | 4 | 0 | moved:gameplay,helper:step_timeline_phase_transition,uses:EventBus |
...
| `core/rules/employee_rules/salary.gd` | 58 | 4 | 0 | uses:PlayerStateAccess |
...
| `core/state/state_updater/employees_and_milestones.gd` | 121 | 2 | 0 | uses:PlayerStateAccess |
...
- `core/rules/employee_rules/salary.gd`：（已整改 2026-01-27）营销员免薪（`marketing_no_salary`）判定改为复用 `MilestoneEffectQueries`（减少 milestones->effects 遍历样板）；（已整改 2026-01-27）读取 `player.milestones` 改为复用 `PlayerStateAccess`（减少重复/样板）；其余未发现明显结构问题（小文件/职责相对单一）
...
- `core/state/state_updater/employees_and_milestones.gd`：（已整改 2026-01-27）读取 `player.milestones` 改为复用 `PlayerStateAccess`（减少重复/样板）；其余未发现明显结构问题（小文件/职责相对单一）
