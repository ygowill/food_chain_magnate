...
- `MilestoneRaceAnalyzer` 只基于公开 `milestone_pool_public`、己方已获得里程碑、公开 round counters 和候选动作触发类型做弱评估；当前覆盖 `first_train`、第三次招聘触发的 `first_hire_3`、基础生产、Errand Boy/Cart、飞机营销、基础营销与降价类里程碑，并会把 `gain_cards`、营销里程碑的 `sell_bonus` 和飞机营销的 `turnorder_empty_slots` effect 计入弱估值，把 `milestone_race_value` / `milestone_race_ids` 写入 scorer features。它不会读取隐藏对手状态，也不会假设未公开结构。
