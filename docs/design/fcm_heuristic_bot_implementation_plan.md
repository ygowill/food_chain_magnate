...
- `StrategyScorer` 已给房屋/花园扩张链路增加显式路线价值：当公开地图仍有 `house_number_supply_remaining`，或仍可给无花园房屋加花园，且己方尚无已上岗的 `use:place_house` / `use:add_garden` 员工时，`management_trainee`、支撑其训练的 `trainer`、`management_trainee -> new_business_developer` 培训，以及把 reserve 中的 NBD 上岗都会获得 `*_placement_route_value`。该价值受经济就绪门槛约束：如果当前没有公开需求且现金不足以支付带薪 NBD 的下一次 Payday，房屋路线只保留低强度前置价值，避免在新需求转化为收入前训练出 NBD 又立刻被裁掉。如果房屋/花园已无增长空间，或己方已经有上岗 NBD 这类 placement 员工，该路线价值归零，避免无目标地继续投入管理培训路线。
...
- `core/tests/ai/strategy_bot_test.gd`：两个 `StrategyBot` 在 2p base、固定 seed 下跑到至少第 3 轮或 GameOver，并校验同 seed 行动 trace deterministic、strategy trace 元数据、profile 数据加载、营销空覆盖候选过滤、营销商品候选排序、营销可服务房屋/活跃供给评分、招聘 roster 饱和度、房屋扩张路线招聘/培训评分、收入缺口、生产补缺口/供给数量/路线饮料商品推断/过量库存惩罚、PricingPipeline 价格动作评分、关键里程碑 race 评分、冰箱保留、餐厅位置/road graph 评分、房屋放置距离评分、Payday 解雇评分等特征。
