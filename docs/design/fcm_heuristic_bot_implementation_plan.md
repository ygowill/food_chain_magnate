...
- `tools/run_bot_selfplay_matrix.gd` / `tools/run_bot_selfplay_matrix.sh` 在同一批 seed 下顺序运行多个 `--config=`（例如 `--config=strategy --config=osla --config=random,strategy`），合并 JSONL 并直接生成 summary，作为后续网格/随机调参的最小矩阵入口。
