...
- `StrategyScorer` 已给房屋/花园扩张链路增加显式路线价值：当公开地图仍有 `house_number_supply_remaining`，或仍可给无花园房屋加花园，且己方尚无 `use:place_house` / `use:add_garden` 员工时，`management_trainee`、支撑其训练的 `trainer`、以及 `management_trainee -> new_business_developer` 培训会获得 `*_placement_route_value`。如果房屋/花园已无增长空间，或己方已经拥有 NBD 这类 placement 员工，该路线价值归零，避免无目标地继续投入管理培训路线。
