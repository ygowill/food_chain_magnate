...
- `StrategyScorer` 对 `train` 候选记录 `train_capacity_upgrade_value`、`train_capacity_upgrade_products`、来源/目标产量与对应供应价值差。训练评分复用现有 `product_supply_action_value` 判断当前/规划缺口，只对目标员工相对来源员工的有效产能增量加分；因此在已有基础供给、可服务食品需求明显超过普通厨师产能时，`burger_cook -> burger_chef` 会压过继续补普通 `burger_cook`，但无需求或过量库存时不会无条件追高级厨师。
...
- `core/tests/ai/strategy_bot_scenario_benchmark_test.gd`：StrategyBot 的路线级场景基准。它不替代 `StrategyBotTest` 的组件断言，而是把关键策略断点抽成可命名、可扩展的 deterministic benchmark，作为后续 StrategyBot 开发的主验收入口。当前已覆盖营销本回合发起、Marketing 结算生成需求、下轮结构补产能、训练补食品基础产能、训练升级食品产能、首次 Errand Boy 饮料产量、路线型饮料采购并在 Dinnertime 销售、生产并在 Dinnertime 销售的无冰箱常规收入链，价格支持招聘/上岗/真实价格行动闭环，waitress 在稳定收入后的招聘/上岗路线，DinnerPreview 对现金/销售员工等 Dinnertime 新增公开里程碑的评分，以及 base 支持型里程碑 effect 估值不退化。
