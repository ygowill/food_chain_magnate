...
- 后续计划：暂停继续扩大 MCTS 宽度，先从 hints / evaluator 侧定位为什么高预算下会减少早期营销、生产和招聘。下一步应优先检查 `price_recovery` / `supply_capacity` / 饮料路线是否在未形成稳定现金前拿到过高 plan value，以及食品营销收入线是否缺少早期现金保全加权。
