...
- 进展：`StrategicPlanHints` 现在只会在目标产品真的包含饮料时才注入 `procure_drink` / `procure_drinks`，而 `StrategicPlanEvaluator` 的 `route_transition_bonus` 也会先等到现金站稳后再奖励从 `marketing_income` 切到 `price_recovery` / `supply_capacity`。下一步转入 MCTS 的预算与展开策略调优。
