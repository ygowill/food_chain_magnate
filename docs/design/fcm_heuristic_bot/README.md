...
- 日期：2026-05-11
- 提交：`a8604415 feat(ai): carry strategic route history`
...
- 当前状态：`StrategyBot` 的 Restructuring trace 已补专用摘要字段，能直接看到 `restructuring_*` 候选数量、`restructure_employee` / `set_company_structure_direct` / `submit_restructuring` 的 best score，以及 `score_gap_to_submit`。基于 timeout seed 的 trace，`StrategyStructurePlanner` 现已把结构动作里的泛员工分降权为 `structure_employee_weight=0.35`，保留结构激活、价格路线、waitress 路线和扩张 readiness 的完整价值：`recruiting_girl` 这类边际直改应先让位于 `submit_restructuring`，但 `kitchen_trainee` / `pricing_manager` 这类能打开收入路线的结构动作仍可压过 submit。
- 当前状态：`CandidateGenerator` 的 restructuring 保留判定现在会忽略 `plan_hints`，避免像 `campaign_manager -> brand_manager` 这类只靠长期路线偏置支撑的 optional training 把 `restructure_employee` 拉成 reserve 循环；真实训练保留仍由 base training route / supply need 决定，`marketing_trainee` 这类有实际解除封锁价值的保留路径不受影响。2026-05-11 复跑 `12352/12353/12354`（1s/decision、180s 单局上限）后，三局全部在 `GameOver` 结束，平均 `round=10.333`、`steps=189.0`、`budget_expired_rate=0.245`，说明这次收口确实把 Restructuring 的死循环压下去了。
...
- 当前状态：`StrategicMCTSSearch` 现在会对 non-root 的 `route_stalled` 分支做提前止损；`_select_best_child` 不再继续挑选 stalled non-root child，`_select_leaf` 遇到 stalled non-root leaf 也会直接收口，不再继续 populate / expand 这条分支。`StrategicPlanTest` 已补回归，确认 stalled leaf 不再消耗展开预算。
- 验证：本轮 stalled branch prune 后，Godot 4.6 console `res://tools/check_compile.gd` 得到 `PASS files=1234`，`res://ui/scenes/tests/all_tests.tscn -- --autorun` 以 exit code `0` 完成。随后用正确 `--script` 入口跑 `strategy,strategic` / `base_revenue_growth_v1` / 1-match `r8` / seed `12345` / `2s` `play` MCTS smoke，`success=1.0`，`SEARCH` 里 `strategic_total=28`、`strategic_cached=1`、`time_ms_avg=417.306`、`budget_expired_rate=0.058`，`MCTS_ROUTE` 为 `route_switch_avg=0.008`、`non_root_populated_avg=0.405`、`non_root_expanded_avg=0.545`、`non_root_candidate_avg=1.256`，route mix 为 `marketing_income=20`、`supply_capacity=9`。这说明 stalled prune 已落到实际搜索成本控制里，但 `play` 档仍需要继续收预算触发和展开宽度。
...
- 追加诊断：按 `strategy,strategy` vs `strategy,strategic` 的 3-match `r8` / `base_revenue_growth_v1` smoke 复跑后，`strategy_vs_strategic-play-mcts-d4-r1-p2-s20-b16-m1-mi6-md2-mk2-me125-mw25-mv2` 的 `MCTS_ROUTE` 仍是 `route_switch_avg=0.000`、`non_root_populated_avg=0.000`、`non_root_expanded_avg=0.000`、`non_root_candidate_avg=0.000`，但 selected route types 已出现 `marketing_income=54`、`price_recovery=25`、`product_switch_attack=3`、`supply_capacity=6`。与 baseline 相比，`cash_min_after_first_positive` 没有变化，`avg_steps_delta=+3.333`，`tuning_score_delta=-1.788`；这说明 route-history wiring 已经接通，但当前配置还没有把路线切换真正推高。
- 追加诊断：同样对照下改用 2s `play` 预算和更宽 MCTS（`d8-r2-p4-s40-b240-m1-mi12-md3-mk4`）后，非根展开开始出现：`route_switch_avg=0.033`、`non_root_populated_avg=0.428`、`non_root_expanded_avg=0.695`、`non_root_candidate_avg=1.255`，route types 为 `marketing_income=47`、`price_recovery=8`、`supply_capacity=10`。但这组 `strategic` 对 baseline 的 `tuning_score_delta=-62.375`，`cash_min_after_positive_delta=[0.0, 0.0]`，`cash_max_seen_delta=[-28.5, 1.5]`，行动分布少了 `initiate_marketing=-1.5`、`produce_food=-3.0`、`recruit=-1.5`，多了 `procure_drinks=+1.5`。结论：预算变宽能让 Plan MCTS 下钻，但当前 evaluator / hints 仍会把路线从早期食品营销收入拉向收益不够稳定的补位路线，不能继续只靠扩大搜索宽度。
...
- 进展：早期收入权重第一轮已收紧。`StrategicPlanGenerator` 在当前现金低于 15 时不再因为 route history 直接鼓励从 `marketing_income` 切到 `price_recovery` / `supply_capacity`，而是保留 `marketing_income` 作为早期收入路线；`StrategicPlanEvaluator` 同步把现金站稳门槛提高到 15，并在未站稳前提高营销收入路线的 demand created / sold 与 `initiate_marketing` 进度权重。`StrategicPlanTest` 已补低现金 route-history 回归，避免后续改动重新把低现金局面推向补位路线。
- 验证：本轮使用 Godot 4.6 console 得到 `CheckCompile PASS files=1234`，`AllTests PASS 425/425`。`strategy,strategy` vs `strategy,strategic` 的 1-match `r8` / seed 12345 / `base_revenue_growth_v1` / 2s 单边 strategic smoke 为 success `1.0`、failures `0`，`cash_min_after_first_positive=[10,10]`，`strategic` 侧 `MCTS_ROUTE` 为 `route_switch_avg=0.000`、`non_root_populated_avg=0.342`、`non_root_expanded_avg=0.417`、`non_root_candidate_avg=0.683`，route types 为 `marketing_income=13`、`supply_capacity=2`；但 `cash_max_seen_delta=[-44.0,0.0]`，双 strategic 2s `play` smoke 超过 5 分钟仍未完成。结论：早期现金底线没有退化，但当前 `play` 预算/展开策略成本过高且后期现金峰值仍弱，下一步进入 MCTS 预算与展开策略调优。
...
- 20 局 `strategy` vs `strategic` 对照中那 4 局未完成局全部集中在 `Restructuring`；其中 part1 的 seed `12353` 在 `max_steps=1000` 时已经累计到 `restructure_employee=455`、`set_company_structure_direct=477`，part2 的 seed `12352/12353/12354` 也都在 600s timeout 前停留在同一阶段。这个模式更像结构编辑循环，而不是自然平局。
