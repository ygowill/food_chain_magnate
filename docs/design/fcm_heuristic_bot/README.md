...
- 验证：使用 `D:\tools\Godot_v4.6-stable_win64.exe\Godot_v4.6-stable_win64_console.exe` 运行 `res://tools/check_compile.gd` 得到 `PASS files=1234`；运行 `res://ui/scenes/tests/all_tests.tscn -- --autorun` 得到 `425/425 PASS`、`StrategicPlanTest PASS (1794ms)`、`BotSelfplayToolTest PASS (973ms)`、`BotSelfplayMatrixTest PASS (1433ms)`、`total_ms=93659`。Godot headless 退出时仍会输出既有 RID/resource leak warning；以 AllTests summary 为准。
