...
- `StrategicPlanRunner` 会把 route history 原样带回 rollout payload，`StrategicPlanEvaluator` 则通过 `route_transition_bonus` 把“切换路线”或“重复同类路线”的差异计入 plan value，且会先等到现金站稳后再奖励从 `marketing_income` 切到 `price_recovery` / `supply_capacity`，避免 early-income 阶段被补位路线过早拉偏。结合 20 局 post-restructure 对照里 `price_recovery` 偏多、`supply_capacity` 偏少的结果，现金站稳后现在会把 `marketing_income -> supply_capacity` 的 transition bonus 设得略高于 `marketing_income -> price_recovery`。
