...
- Point 14C.1 commit: `fix(ai): keep same-command strategic fallback` (`fb833106`).
- Point 14C.2 target: stop searching for a marketing-opening natural positive unless a deterministic different-command fixture proves one. The next implementation point must try a different narrow route with complete rollouts, starting with `price_recovery` after positive cash footing because it can produce concrete proof through `lost_to_competitor` reduction, `demand_sold`, `cash_after`, or `cash_max_seen` without relaxing gates.
- Point 14C.2 fixture plan:
  - Build or reuse a deterministic engine fixture where the current legal action set allows a non-baseline price route step, the precomputed `StrategyBot` command is different, and a `price_recovery` plan can complete to `phase_boundary` or `round_horizon` without `budget_expired`.
  - Require the compared candidate to pass existing hard gates, clear `min_delta_score=12`, and show at least one existing economic proof field over baseline. Do not add new proof categories for this point.
  - If `price_recovery` is also StrategyBot-dominated under a real fixture, document that result and move to `supply_capacity` only after positive cash footing; do not tune thresholds or restore route-only credit.
  - Add focused test coverage for both the accepted different-command fixture and the paired negative same-command or route-only fallback case before any commit.
- Point 14C.2 design check before implementation: aligned. This continues the final design rather than a transitional scheme: strategic search may only add strength by proving a different legal command with completed rollout economics; fallback remains the exact precomputed `StrategyBot` command.
- Point 14C.2 diagnostic: tested deterministic `price_recovery` windows in `GetFood`, `Recruit`, and `Restructuring` using the existing contested-price fixture shape. Results:
  - `GetFood` with zero own burger inventory still has StrategyBot choose `set_price` (`mandatory_set_price`), so the current command is not different.
  - `Recruit` without an owned pricing manager still has StrategyBot recruit `pricing_manager`, even with no inventory or a missing campaign manager, so the route is StrategyBot-dominated before StrategicBot can add value.
  - `Restructuring` with an empty CEO slot and `pricing_manager`/`waitress` competitors still has StrategyBot assign `pricing_manager`. With full slots, the rollout cannot prove a complete price path.
- Point 14C.2 follow-up diagnostic: moved to `supply_capacity` after positive cash footing. Empty-slot restructuring variants with `burger_cook`/`burger_chef` versus `pricing_manager` showed StrategyBot already assigns the supply employee when the inventory gap is large enough. The compared candidate and baseline command streams were behavior-identical, so the same-command guard correctly keeps the result as StrategyBot fallback.
- Point 14C.2 implementation target from diagnostics: harden the evidence boundary discovered by the fixtures. Compared rollouts must stop at a complete comparison boundary (`phase_boundary`, `round_horizon`, or `game_over`) to be accepted. `budget_expired` remains its own explicit rejection, and other incomplete stops such as `bot_failed` are now rejected as `candidate_rollout_incomplete` or `baseline_rollout_incomplete`.
- Point 14C.2 design check before verification: aligned. This narrows StrategicBot authority and preserves the final design rule that every override needs a different legal command plus complete-rollout economic proof. No score threshold, hard-gate relaxation, route-only credit, or new proof category was added. The correct next strength work is a new route fixture; `price_recovery` and same-round `supply_capacity` are documented as StrategyBot-dominated under the tested real fixtures.
...
- Point 14C.3 product-switch diagnostic: tested three real-engine marketing windows based on the existing opponent-capacity-gap scenario: pizza-only supply, burger-plus-pizza supply, and two burger demands. In all three, `StrategyBot` already chose the pizza `campaign_manager` billboard product-switch command, and compared candidate rollouts had the same command stream as baseline with `delta_below_threshold`.
- Point 14C.3 product-switch design check: aligned. Because the current command is not different and the economic summaries are behavior-identical, this route cannot be accepted as StrategicBot strength. Do not tune product-switch scoring or weaken same-command fallback.
- Point 14C.4 target: move to `growth`. The fixture must require a different current command from StrategyBot and a completed rollout that proves existing economic movement, not just placement-route intent. If growth is also StrategyBot-dominated, the next implementation should stop adding route families and instead improve the search objective around a proven blind spot from decision traces.
