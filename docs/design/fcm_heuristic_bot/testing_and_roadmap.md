...
- `core/tests/ai/strategic_plan_test.gd`：现在额外覆盖 plan generator 的 route-history bias，保证非根路线展开会轻微偏向 `supply_capacity` / `price_recovery` 等补位路线，而不是继续重复同类 `marketing_income` 路线。
